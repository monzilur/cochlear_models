function [X_ft, t, params] = cochleagram_spec_Hill(x, fs, dt, type, varargin)
% function [X_ft, t, params] = cochleagram_spec_Hill(x, fs, dt, type, varargin)
%
...
for tt = 1:size(spec,2)
    X(:,tt) = max(10*log10(params.melbank.x*(abs(spec(params.melbank.na:params.melbank.nb,tt)).^2)),...
        params.threshold) - params.threshold;
end
SAT = 0.16; % This was found from crossvalidation
X_ft = hill_function(X,1.77,1e-2,SAT);

% median_X_ft = median(X_ft,2);
% median_X_ft(median_X_ft == 0) = 1e-30;
% X_ft = X_ft ./ median_X_ft;

%X_ft = hill_function(X,hill_exponent,1);
