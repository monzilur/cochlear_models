...
function [filters, CenterFreqs, gains, EarQ] = DesignLyonFilters(fs,topf,...
    lowf,NumberOfChannels,StepFactor)
...
% 
topf = topf + sqrt(topf^2+Eb^2)*StepFactor*EarZeroOffset - ...
                     sqrt(topf^2+Eb^2)*StepFactor;
...
EarQ = NumberOfChannels*StepFactor/(-log(lowf + sqrt(lowf^2 + Eb^2)) + ...
          log(topf + sqrt(Eb^2 + topf^2)));
